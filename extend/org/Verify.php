...
class Verify
{
    ...
    public function check($code, $id = '')
    {
        ...
        // 验证码不能为空
        if($id == 'redis'){
        	$secode = Cache::store('redis')->get($key);
//         	var_dump($secode);die;
        }else{
        	$secode = session($key);
        }
    ...
    public function entry($id = '')
    {
        ...
        if($id == 'redis'){
        	Cache::store('redis')->set($key.$id,$secode);
        }else{
    ...
    private function clearSession($sessionName,$key,$isRest  = false){
    	if($sessionName == 'redis'){
